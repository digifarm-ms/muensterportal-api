...
@dataclass
class _SpyWikiRetriever:
    """Canned wiki results + records (query, top_k) per call."""
...
@dataclass
class _SpyWebSearcher:
    """Canned web results + records (query, max_results) per call."""
...
@dataclass
class _FakeReranker:
    """Deterministic reranker: sorts candidates by content length descending."""
...
def test_oversample_factor_drives_candidate_k():
    wiki = _SpyWikiRetriever(results=[])
    web = _SpyWebSearcher(results=[])
    ...
    assert wiki.calls == [("anything", 12)]
    assert web.calls == [("anything", 12)]


def test_retrieve_pools_wiki_and_web_then_reranks():
    wiki = _SpyWikiRetriever(
        results=[
