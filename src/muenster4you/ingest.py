...
from muenster4you.embedder import load_embedder
...
def load_pages_from_media_wiki(sqlite: SQLiteMediaWiki, namespace: int) -> Iterable[BaseWikiPage]:
...
def add_embeddings(
    ...
    for batch in batched(pages, batch_size, strict=False):
        ...
        embeddings = embedder.encode(texts, prompt_name="document", show_progress_bar=False)
...
def ingest(
    ...
    embedder = load_embedder(embedding_model, device=device)
